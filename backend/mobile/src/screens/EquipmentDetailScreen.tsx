  ...
  if (isError) {
    return (
      <View style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}>
        <Ionicons name="warning-outline" size={48} color={colors.semantic.critical} />
        <Text style={{ color: colors.text.primary, marginTop: 16 }}>Failed to load data.</Text>
        <Text style={{ color: colors.text.secondary, marginTop: 8 }}>{String(error)}</Text>
        <TouchableOpacity onPress={() => refetch()} style={[styles.askButton, {marginTop: 24}]}>
          <Text style={styles.askButtonText}>Retry</Text>
        </TouchableOpacity>
      </View>
    )
  }
    ...
    >
      ...
      <View style={styles.statsGrid}>
        <Card style={styles.statCard}>
          ...
          <Text style={[styles.statValue, eq.failures > 3 && { color: colors.semantic.critical }]}>
            {eq.failures}
          </Text>
        </Card>
        ...
        {eq.properties && Object.keys(eq.properties).length > 0 ? (
          Object.entries(eq.properties)
            .filter(([key]) => !['tag', 'name', 'type', 'status', 'equipment_type'].includes(key))
            .slice(0, 5) // Show up to 5 properties
            .map(([key, value]) => (
          ...
          [
            { label: 'Max Temp', value: eq.max_temp ? `${eq.max_temp}°C` : 'N/A' },
            { label: 'Max Pressure', value: eq.max_pressure ? `${eq.max_pressure} PSI` : 'N/A' },
            { label: 'Max Flow', value: eq.max_flow ? `${eq.max_flow} m³/h` : 'N/A' },
          ].map(({ label, value }) => (
